...
import type { ExplorerData, ExplorerBlock } from "@/types/explorer";
...
function parseExplorerPayload(payload: unknown): ExplorerData {
    ...
    return {
        ...
        blocks: Array.isArray(value.blocks)
            ? value.blocks.map((block): ExplorerBlock => ({
                ...
                txCount: Math.max(0, asNumber(block?.txCount ?? block?.header?.numTxes)),
                blockWeight: Math.max(0, asNumber(block?.blockWeight ?? block?.header?.blockWeight)),
                ...
                header: {
                    ...
                    difficulty: asNumber(block?.header?.difficulty ?? block?.difficulty),
                    reward: asNumber(block?.header?.reward ?? block?.reward),
                    numTxes: Math.max(0, asNumber(block?.header?.numTxes ?? block?.txCount)),
                    ...
                    cumulativeDifficulty: asNumber(block?.header?.cumulativeDifficulty),
                    blockWeight: Math.max(0, asNumber(block?.header?.blockWeight ?? block?.blockWeight)),
                    prevHash: String(block?.header?.prevHash ?? block?.prevHash ?? ""),
